...
bool CUpDownClient::ProcessHelloPacket(const bytes* pachPacket, uint32 nSize)
{
...
bool CUpDownClient::ProcessHelloAnswer(const bytes* pachPacket, uint32 nSize)
{
	const CMemFile data(pachPacket,nSize);
...
bool CUpDownClient::ProcessMuleInfoPacket(const bytes* pachPacket, uint32 nSize)
{
...
void CUpDownClient::ProcessMuleCommentPacket(const bytes* pachPacket, uint32 nSize)
{
	if (!m_reqfile) {
...
void CUpDownClient::ProcessSharedFileList(const bytes* pachPacket, uint32 nSize, wxString& pszDirectory)
{
	if (m_iFileListRequested > 0) {
...
void CUpDownClient::ProcessPublicKeyPacket(const bytes* pachPacket, uint32 nSize)
{
...
void CUpDownClient::ProcessSignaturePacket(const bytes* pachPacket, uint32 nSize)
{
...
void CUpDownClient::ProcessSecIdentStatePacket(const bytes* pachPacket, uint32 nSize)
{
	if ( nSize != 5 ) {
		return;
	}
...
void CUpDownClient::ProcessPublicIPAnswer(const bytes* pbyData, uint32 uSize)
{
	if (uSize != 4) {
...
void CUpDownClient::SetBuddyID(const bytes* pucBuddyID)
{
	if( pucBuddyID == NULL ){
		md4clr(m_achBuddyID);
		...
		if (GetMessagesSent() == 0 && GetMessagesReceived() == 0 && GetChatCaptchaState() != CA_CAPTCHASOLVED) {
			...
			if (GetChatCaptchaState() != CA_CHALLENGESENT) {
				...
				if (m_fSupportsCaptcha) {
					...
					if (m_cCaptchasSent < 3) {	// no more than 3 tries
						...
						if (captcha.WriteCaptchaImage(memstr)){
							...
							m_cCaptchasSent++;
							CMemFile fileAnswer((bytes*) memstr.GetOutputStreamBuffer()->GetBufferStart(), memstr.GetLength());
				...
				if (m_strCaptchaChallenge.CmpNoCase(message.Trim().Right(std::min(message.Length(), m_strCaptchaChallenge.Length()))) == 0) {
					...
					bytes statusResponse = 0; // status response
					...
					CPacket* packet = new CPacket(OP_CHATCAPTCHARES, 1, OP_EMULEPROT, false);
					bytes statusResponse = (m_cCaptchasSent < 3) ? 1 : 2; // status response
					packet->CopyToDataBuffer(0, &statusResponse, 1);
					theStats::AddUpOverheadOther(packet->GetPacketSize());
					SafeSendPacket(packet);
