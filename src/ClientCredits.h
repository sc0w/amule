...
public:
	...
	uint16		nReserved3;
	uint8		nKeySize;
	bytes		abySecureIdent[MAXPUBKEYSIZE];
};
...
public:
	...
	const bytes*	GetSecureIdent() const	{return m_abyPublicKey;}
	...
	void	SetLastSeen();
	bool	SetSecureIdent(const bytes* pachIdent, uint8 nIdentLen); // Public key cannot change, use only if there is not public key yet
...
private:
	...
	bytes			m_abyPublicKey[80];			// even keys which are not verified will be stored here, and - if verified - copied into the struct
