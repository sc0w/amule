...
uint8 CClientCreditsList::CreateSignature(CClientCredits* pTarget, bytes* pachOutput, uint8 nMaxSize, uint32 ChallengeIP, uint8 byChaIPKind, void* sigkey)
{
	...
	try {
		...
		CryptoPP::AutoSeededX917RNG<CryptoPP::DES_EDE3> rng;
		bytes abyBuffer[MAXPUBKEYSIZE+9];
...
bool CClientCreditsList::VerifyIdent(CClientCredits* pTarget, const bytes* pachSignature, uint8 nInputSize, uint32 dwForIP, uint8 byChaIPKind)
{
	wxASSERT( pTarget );
	...
	bool bResult;
	try {
		CryptoPP::StringSource ss_Pubkey((bytes*)pTarget->GetSecureIdent(),pTarget->GetSecIDKeyLen(),true,0);
