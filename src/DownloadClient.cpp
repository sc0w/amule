...
void CUpDownClient::ProcessHashSet(const bytes* packet, uint32 size)
{
...
void CUpDownClient::ProcessBlockPacket(const bytes* packet, uint32 size, bool packed, bool largeblocks)
{
		...
		for (; it != m_PendingBlocks_list.end(); ++it) {
			...
			if ((cur_block->block->StartOffset <= nStartPos) && (cur_block->block->EndOffset >= nStartPos)) {
				...
				if (!packed) {
					...
					lenWritten = m_reqfile->WriteToBuffer( size - header_size, (bytes*)(packet + header_size), nStartPos, nEndPos, cur_block->block, this);
				} else {
					// Packed
					...
					bytes *unzipped = new bytes[lenUnzipped];

					// Try to unzip the packet
					int result = unzip(cur_block, (bytes*)(packet + header_size), (size - header_size), &unzipped, &lenUnzipped);
...
int CUpDownClient::unzip(Pending_Block_Struct *block, bytes *zipped, uint32 lenZipped, bytes **unzipped, uint32 *lenUnzipped, int iRecursion)
{
	...
	if (err == Z_STREAM_END) {
		...
		bytes *temp = new bytes[newLength];
...
void CUpDownClient::ProcessAICHAnswer(const bytes* packet, uint32 size)
{
...
void CUpDownClient::ProcessAICHRequest(const bytes* packet, uint32 size)
{
