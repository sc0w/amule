...
void CKnownFile::CreateHashFromHashlist(const ArrayOfCMD4Hash& hashes, CMD4Hash* Output)
{
	...
	std::vector<bytes> buffer(hashes.size() * MD4HASH_LENGTH);
	std::vector<bytes>::iterator it = buffer.begin();
...
void CKnownFile::CreateHashFromInput(const bytes* input, uint32 Length, CMD4Hash* Output, CAICHHashTree* pShaHashOut )
{
