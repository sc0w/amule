...
CMemFile::CMemFile(bytes* buffer, size_t bufferSize)
{
...
CMemFile::CMemFile(const bytes* buffer, size_t bufferSize)
{
	MULE_VALIDATE_PARAMS(buffer, wxT("CMemFile: Attempted to attach invalid buffer."));

	m_buffer		= const_cast<bytes*>(buffer);
	m_BufferSize	= bufferSize;
	m_fileSize		= bufferSize;
	m_growthRate	= 0;
	m_position		= 0;
...
void CMemFile::enlargeBuffer(size_t size)
{
	...
	bytes *tmp = (bytes*)realloc(m_buffer, newsize);
	if (tmp) {
		m_buffer = tmp;
