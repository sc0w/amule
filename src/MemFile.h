...
public:
	 ...
	 *
	 * If the buffer is a const bytes*, the memfile is read-only.
	 */
	CMemFile(bytes* buffer, size_t bufferSize);
	CMemFile(const bytes* buffer, size_t bufferSize);
	...
	bytes* GetRawBuffer() const { return m_buffer; }
