...
// Returns a BASE16 encoded bytes array
//
// [In]
//   buffer: Pointer to bytes array
//   bufLen: Lenght of buffer array
//
// [Return]
//   wxString object with BASE16 encoded bytes array
...
// Decodes a BASE16 string into a bytes array
//
// [In]
...
//
// [Out]
//   buffer: bytes array containing decoded string
unsigned int DecodeBase16(const wxString &base16Buffer, unsigned int base16BufLen, bytes *buffer)
{
	if (base16BufLen & 1) {
		return 0;
	}
	...
	for(unsigned int i = 0; i < base16BufLen; ++i) {
		...
		bytes word = (lookup < 0 || lookup >= BASE16_LOOKUP_MAX) ?
...
// Returns a BASE32 encoded bytes array
//
// [In]
//   buffer: Pointer to bytes array
//   bufLen: Lenght of buffer array
//
// [Return]
//   wxString object with BASE32 encoded bytes array
wxString EncodeBase32(const unsigned char* buffer, unsigned int bufLen)
{
	...
	for(i = 0, index = 0; i < bufLen;) {
		// Is the current word going to span a bytes boundary?
		if (index > 3) {
...
// Decodes a BASE32 string into a bytes array
//
// [In]
