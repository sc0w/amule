...
CPacket::CPacket(CPacket &p)
{
	...
	tempbuffer	= NULL;
	if (p.completebuffer) {
		completebuffer	= new bytes[size + 10];;
		pBuffer	= completebuffer + sizeof(Header_Struct);
	} else {
		completebuffer	= NULL;
		if (p.pBuffer) {
			pBuffer = new bytes[size];
		} else {
			pBuffer = NULL;
		}
	}
...
CPacket::CPacket(bytes* rawHeader, bytes *buf)
{
	memset(head, 0, sizeof head);
...
CPacket::CPacket(const CMemFile& datafile, uint8 protocol, uint8 ucOpcode)
{
	...
	memset(head, 0, sizeof head);
	tempbuffer = NULL;
	completebuffer = new bytes[size + sizeof(Header_Struct)/*Why this 4?*/];
	pBuffer = completebuffer + sizeof(Header_Struct);
...
CPacket::CPacket(int8 in_opcode, uint32 in_size, uint8 protocol, bool bFromPF)
{
	...
	tempbuffer	= NULL;
	if (in_size) {
		completebuffer = new bytes[in_size + sizeof(Header_Struct) + 4 /*Why this 4?*/];
		pBuffer = completebuffer + sizeof(Header_Struct);
...
CPacket::CPacket(bytes* pPacketPart, uint32 nSize, bool bLast, bool bFromPF)
{
...
uint32 CPacket::GetPacketSizeFromHeader(const bytes* rawHeader)
{
...
void CPacket::CopyToDataBuffer(unsigned int offset, const bytes* data, unsigned int n)
{
	wxASSERT(offset + n <= size + 1);
	memcpy(pBuffer + offset, data, n);
}

bytes* CPacket::GetPacket() {
	if (completebuffer) {
		if (!m_bSplitted) {
		...
		tempbuffer = new bytes[size + sizeof(Header_Struct) + 4 /* why this 4?*/];
...
bytes* CPacket::DetachPacket() {
	if (completebuffer) {
		if (!m_bSplitted) {
			memcpy(completebuffer, GetHeader(), sizeof(Header_Struct));
		}
		bytes* result = completebuffer;
		...
		memcpy(tempbuffer,GetHeader(),sizeof(Header_Struct));
		memcpy(tempbuffer+sizeof(Header_Struct),pBuffer,size);
		bytes* result = tempbuffer;
		tempbuffer = 0;
		return result;
	}
}

bytes* CPacket::GetHeader() {
...
void CPacket::PackPacket()
{
	...
	bytes* output = new bytes[newsize];
...
bool CPacket::UnPackPacket(uint32 uMaxDecompressedSize) {
	...
	bytes* unpack = new bytes[nNewSize];
