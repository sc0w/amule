...
public:
	...
	CPacket(bytes* header, bytes *buf); // only used for receiving packets
	...
	CPacket(bytes* pPacketPart, uint32 nSize, bool bLast, bool bFromPF = true); // only used for splitted packets!
	...
	static uint32		GetPacketSizeFromHeader(const bytes* rawHeader);
	...
	const bytes*	GetDataBuffer(void) const { return pBuffer; }
	void			Copy16ToDataBuffer(const void* data);
	void			CopyToDataBuffer(unsigned int offset, const bytes* data, unsigned int n);
