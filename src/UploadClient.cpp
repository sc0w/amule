...
void CUpDownClient::CreateStandardPackets(const bytes* buffer, uint32 togo, Requested_Block_Struct* currentblock)
{
	uint32 nPacketSize;
...
void CUpDownClient::CreatePackedPackets(const bytes* buffer, uint32 togo, Requested_Block_Struct* currentblock)
{
	uLongf newsize = togo+300;
	CScopedArray<bytes> output(newsize);
...
void CUpDownClient::ProcessRequestPartsPacket(const bytes* pachPacket, uint32 nSize, bool largeblocks) {
