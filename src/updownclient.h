...
public:
	...
	void		ProcessSharedFileList(const bytes* pachPacket, uint32 nSize, wxString& pszDirectory);
	...
	bool		ProcessHelloAnswer(const bytes* pachPacket, uint32 nSize);
	bool		ProcessHelloPacket(const bytes* pachPacket, uint32 nSize);
	void		SendHelloAnswer();
	bool		SendHelloPacket();
	void		SendMuleInfoPacket(bool bAnswer, bool OSInfo = false);
	bool		ProcessMuleInfoPacket(const bytes* pachPacket, uint32 nSize);
	void		ProcessMuleCommentPacket(const bytes* pachPacket, uint32 nSize);
	...
	void		ProcessRequestPartsPacket(const bytes* pachPacket, uint32 nSize, bool largeblocks);
	void		ProcessRequestPartsPacketv2(const CMemFile& data);

	void		SendPublicKeyPacket();
	void		SendSignaturePacket();
	void		ProcessPublicKeyPacket(const bytes* pachPacket, uint32 nSize);
	void		ProcessSignaturePacket(const bytes* pachPacket, uint32 nSize);
	uint8		GetSecureIdentState();

	void		SendSecIdentStatePacket();
	void		ProcessSecIdentStatePacket(const bytes* pachPacket, uint32 nSize);
	...
	void		ProcessHashSet(const bytes* packet, uint32 size);
	bool		AddRequestForAnotherFile(CPartFile* file);
	bool		DeleteFileRequest(CPartFile* file);
	void		DeleteAllFileRequests();
	void		SendBlockRequests();
	void		ProcessBlockPacket(const bytes* packet, uint32 size, bool packed, bool largeblocks);
	...
	void		ProcessAICHAnswer(const bytes* packet, uint32 size);
	void		ProcessAICHRequest(const bytes* packet, uint32 size);
	void		ProcessAICHFileHash(CMemFile* data, const CPartFile* file);

	EUtf8Str	GetUnicodeSupport() const;

	// Barry - Process zip file as it arrives, don't need to wait until end of block
	int		unzip(Pending_Block_Struct *block, bytes *zipped, uint32 lenZipped, bytes **unzipped, uint32 *lenUnzipped, int iRecursion = 0);
	...
	void		ProcessPublicIPAnswer(const bytes* pbyData, uint32 uSize);
	...
	/* Kad buddy support */
	// ID
	const bytes*	GetBuddyID() const		{ return m_achBuddyID; }
	void		SetBuddyID(const bytes* m_achTempBuddyID);
