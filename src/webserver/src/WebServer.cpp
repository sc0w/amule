...
CDynPngImage::CDynPngImage(int w, int h) : CAnyImage(w, h)
{
	...
	//
	m_img_data = new png_mule_byte[3*m_width*m_height];
	memset(m_img_data, 0, 3*m_width*m_height);
	m_row_ptrs = new png_mule_bytep [m_height];
...
void CDynPngImage::png_write_fn(png_structp png_ptr, png_mule_bytep data, png_size_t length)
{
...
unsigned char *CDynPngImage::RequestData(int &size)
{
	...
	png_write_image(png_ptr, (png_mule_bytep *)m_row_ptrs);
...
void CDynProgressImage::DrawImage()
{
	...
	for(int i = 0; i < m_height/2; i++) {
		png_mule_bytep u_row = m_row_ptrs[i];
		png_mule_bytep d_row = m_row_ptrs[m_height-i-1];
	...
	CAnyImage(data->Size(), height), CDynPngImage(data->Size(), height)
{
	...
	// the rendering
	m_background = new png_mule_byte[m_width*m_height*3];
	m_row_bg_ptrs = new png_mule_bytep[m_height];
	for(int i = 0; i < m_height; i++) {
	...
	for(int i = 0; i < m_height; i++) {
		png_mule_bytep u_row = m_row_bg_ptrs[i];
		for(int j = 0; j < m_width; j++) {
...
CNumImageMask::CNumImageMask(int number, int width, int height)
{
	...
	m_row_mask_ptrs = new png_mule_bytep[m_height];
	for(int i = 0; i < m_height; i++) {
		m_row_mask_ptrs[i] = new png_mule_byte[3*m_width];
...
void CNumImageMask::Apply(png_mule_bytep *image, int offx, int offy)
{
	offx *= 3;
	for(int i = 0; i < m_height; i++) {
		png_mule_bytep img_row = image[offy + i];
		png_mule_bytep num_row = m_row_mask_ptrs[i];
...
{
	png_mule_bytep m_row = m_row_mask_ptrs[off*(m_v_segsize-1)];
...
void CNumImageMask::DrawVertLine(int offx, int offy)
{
	for(int i = 0; i < m_v_segsize; i++) {
		png_mule_bytep m_row = m_row_mask_ptrs[offy*(m_v_segsize-1)+i];
