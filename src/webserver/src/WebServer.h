...
class CDynPngImage : public virtual CAnyImage {
	...
	protected:
		png_bytesp m_img_data;
		png_bytesp *m_row_ptrs;

		static void png_write_fn(png_structp png_ptr, png_bytesp data, png_size_t length);
...
class CNumImageMask {
	...
	public:
		...
		void Apply(png_bytesp *image, int offx, int offy);
};
...
class CDynStatisticImage : public virtual CDynPngImage {
		...
		// Prepared background
		//
		png_bytesp m_background;
		png_bytesp *m_row_bg_ptrs;
