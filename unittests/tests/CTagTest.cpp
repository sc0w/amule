...
void test_taglist_serialization(TagPtrList & taglist, bytes* packet, uint64 packet_len);
...
struct BLOBValue : std::vector<bytes>
{
	BLOBValue(uint32 _length, const bytes* _ptr)
		: std::vector<bytes > (_ptr, _ptr + _length)
	{}
};

struct BSOBValue : std::vector<bytes>
{
	BSOBValue(uint8 _length, const bytes* _ptr)
		: std::vector<bytes > (_ptr, _ptr + _length)
	{}
};
...
void test_taglist_serialization(TagPtrList& taglist, bytes* packet, uint64 packet_len)
{
	...
	std::vector<bytes> buf(packet_len);
	...
	for (uint64 i = 0; i < packet_len; i++) {
		CONTEXT(wxString::Format(wxT("Comparing serialized bytes #%") wxLongLongFmtSpec wxT("u"), i));

		ASSERT_EQUALS(packet[i], buf[i]);
	}
}

void ReadTagPtrList(TagPtrList& taglist, bytes* packet, uint64 packet_len)
{
...
void check_single_kad_tag(bytes* packet, size_t packet_len, T tagName, V tagValue)
{
...

	{
		...
		for (size_t i = 0; i < packet_len; i++) {
			CONTEXT(wxString::Format(wxT("Comparing bytes #%d"), static_cast<int>(i)));
...
TEST_M(CTag, KadBsob, wxT("Kad: Read/Write BSOB"))
{
	...
	bytes raw_data[] = {0x01, 0x02, 0x03, 0x04};
	{
...
TEST_M(CTag, KadTagNames, wxT("Kad: Test Kad tags (name=string) - write/read every valid tag name"))
{
	...
	std::vector<bytes> packet(packet_len);
