...
TEST(CMemFile, AttachedBuffer)
{
	...
	ASSERT_RAISES(CRunTimeException, new CMemFile(static_cast<const bytes*>(NULL), 1024));
	ASSERT_RAISES(CRunTimeException, new CMemFile(static_cast<bytes*>(NULL), 1024));
}
...
{
	bytes arr[10];
	CMemFile file(const_cast<const bytes*>(arr), sizeof(arr));
